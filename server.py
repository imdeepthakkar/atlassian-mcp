...
def search_issues(jql: str = "assignee=currentUser() ORDER BY created DESC", max_results: int = 10) -> str:
    ...
    response = requests.post(url, auth=get_auth(), headers={"Accept": "application/json", "Content-Type": "application/json"}, json=payload)
    response.raise_for_status()
    return response.text
...
def get_boards(project_key_or_id: str = None) -> str:
    ...
    response = requests.get(url, auth=get_auth(), headers={"Accept": "application/json"}, params=params)
    response.raise_for_status()
    return response.text
